...
class dc_jqaccordion_widget extends WP_Widget {
    ...
    function dc_jqaccordion_widget() {
		...
		$name =			'Hxck accordion menu';
		$desc = 		'Вертекальное кастомное меню';
	...
	<p>
		<label for="<?php echo $this->get_field_id('classDisable'); ?>"><?php _e('Отключить класс:') ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('hoverDelay'); ?>"><?php _e('Задержка наведения:', 'dcjq-accordion'); ?>
		<select name="<?php echo $this->get_field_name('hoverDelay'); ?>" id="<?php echo $this->get_field_id('hoverDelay'); ?>" >
			<option value='0' <?php selected( $hoverDelay, '0'); ?> >Без задержки</option>
			<option value='200' <?php selected( $hoverDelay, '200'); ?> >0.2 сек</option>
			<option value='400' <?php selected( $hoverDelay, '400'); ?> >0.4 сек</option>
			<option value='600' <?php selected( $hoverDelay, '600'); ?> >0.6 сек</option>
			<option value='800' <?php selected( $hoverDelay, '800'); ?> >0.8 сек</option>
			<option value='1000' <?php selected( $hoverDelay, '1000'); ?> >1.0 сек</option>
		</select>
		</label>
	</p>
	<p><label for="<?php echo $this->get_field_id('speed'); ?>"><?php _e('Скорость анимации:', 'dcjq-accordion'); ?>
		<select name="<?php echo $this->get_field_name('speed'); ?>" id="<?php echo $this->get_field_id('speed'); ?>" >
			<option value='slow' <?php selected( $speed, 'slow'); ?> >Медленная</option>
			<option value='normal' <?php selected( $speed, 'normal'); ?> >Нормальная</option>
			<option value='fast' <?php selected( $speed, 'fast'); ?> >Быстрая</option>
		</select>
		</label>
	</p>
	<p><label for="<?php echo $this->get_field_id('skin'); ?>"><?php _e('Тема:', 'dcjq-accordion'); ?>  <?php 
